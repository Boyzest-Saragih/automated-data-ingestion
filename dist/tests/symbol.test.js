...
import {} from "../repositories/symbol.repository.js";
import { DataSourceType } from "../generated/prisma/index.js";
describe("SymbolService", () => {
    let symbolService;
    let mockSymbolRepository;
    ...
    beforeEach(() => {
        ...
        symbolService = new SymbolService(mockSymbolRepository, mockDataSourceRepository, mockAuditLogRepository);
    });
    describe("createSymbol()", () => {
        it("harus berhasil membuat symbol baru, mengembalikan datanya, dan mencatat audit log success", async () => {
            ...
            expect(mockAuditLogRepository.logSuccess).toHaveBeenCalledWith("CREATE_SYMBOL", "Symbol", `Symbol ${mockSymbolRecord.ticker} berhasil didaftarkan pada DataSource ${mockDataSourceRecord.name}`, {
    ...
    describe("toggleSymbolStatus()", () => {
        it("harus memperbarui status isActive symbol dan mencatat audit log info", async () => {
            ...
            mockSymbolRepository.updateStatus.mockResolvedValue(mockUpdateSymbolRecord);
            // Act
            ...
            expect(mockSymbolRepository.updateStatus).toHaveBeenCalledWith("0001", false);
            expect(mockAuditLogRepository.logInfo).toHaveBeenCalledTimes(1);
            expect(mockAuditLogRepository.logInfo).toHaveBeenCalledWith("TOGGLE_SYMBOL_STATUS", "Symbol", `Status symbol ${mockUpdateSymbolRecord.ticker} diubah menjadi INACTIVE`, { ticker: mockUpdateSymbolRecord.ticker, isActive: false }, mockUpdateSymbolRecord.id);
