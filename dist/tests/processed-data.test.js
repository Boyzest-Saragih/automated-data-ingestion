...
describe("ProcessedDataService", () => {
    let processedDataService;
    let mockRawRepository;
    let mockProcessedDataRepository;
    ...
    beforeEach(() => {
        ...
        processedDataService = new ProcessedDataService(mockRawRepository, mockProcessedDataRepository, mockAuditLogService);
    });
    describe("processRawData()", () => {
        it("harus berhasil memproses RawData, meng-upsert candle, meng-update status RawData menjadi SUCCESS, dan mencatat audit log success", async () => {
            ...
            expect(mockAuditLogService.logSuccess).toHaveBeenCalledWith("PROCESS_RAW_DATA", "ProcessedData", "Berhasil memproses 1 candle untuk BTCUSDT", {
                symbol: "BTCUSDT",
                count: 1,
                granularity: "1d",
            }, "raw-uuid-123");
        ...
        it("harus melempar error jika RawData tidak ditemukan", async () => {
            ...
            // Act & Assert
            await expect(processedDataService.processRawData("non-existent-id")).rejects.toThrow("RawData dengan ID non-existent-id tidak ditemukan.");
        });
        it("harus melempar error, meng-update status RawData menjadi FAILED, dan mencatat audit log error jika payload tidak valid", async () => {
            ...
            const consoleErrorSpy = vi
                ...
                .mockImplementation(() => { });
            // Act & Assert
            await expect(processedDataService.processRawData("raw-uuid-123")).rejects.toThrow("Payload JSON pada RawData ID raw-uuid-123 tidak valid.");
            expect(mockRawRepository.updateStatus).toHaveBeenCalledWith("raw-uuid-123", RawDataStatus.FAILED);
            expect(mockAuditLogService.logError).toHaveBeenCalledWith("PROCESS_RAW_DATA", "RawData", "Gagal memproses RawData ID: raw-uuid-123", expect.any(Error), "raw-uuid-123");
        ...
        it("harus meng-update status RawData menjadi FAILED jika repository upsertMany melempar error", async () => {
            ...
            const consoleErrorSpy = vi
                ...
                .mockImplementation(() => { });
            // Act & Assert
            await expect(processedDataService.processRawData("raw-uuid-123")).rejects.toThrow("Database error");
            expect(mockRawRepository.updateStatus).toHaveBeenCalledWith("raw-uuid-123", RawDataStatus.FAILED);
            expect(mockAuditLogService.logError).toHaveBeenCalledWith("PROCESS_RAW_DATA", "RawData", "Gagal memproses RawData ID: raw-uuid-123", dbError, "raw-uuid-123");
            consoleErrorSpy.mockRestore();
        });
    });
    describe("getCandles()", () => {
        it("harus mengembalikan daftar candle berdasarkan query", async () => {
            ...
            // Assert
            expect(mockProcessedDataRepository.findCandles).toHaveBeenCalledWith(query);
    ...
    describe("getLatestCandle()", () => {
        it("harus mengembalikan candle terbaru untuk symbol dan granularity tertentu", async () => {
            // Arrange
            mockProcessedDataRepository.findLatestCandle.mockResolvedValue(mockProcessedCandle);
            // Act
            const result = await processedDataService.getLatestCandle("BTCUSDT", "1d");
            // Assert
            expect(mockProcessedDataRepository.findLatestCandle).toHaveBeenCalledWith("BTCUSDT", "1d");
            expect(result).toEqual(mockProcessedCandle);
        });
    });
});
//# sourceMappingURL=processed-data.test.js.map