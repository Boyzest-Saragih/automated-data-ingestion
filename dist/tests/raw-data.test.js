...
describe("RawDataIngestionService", () => {
    let rawDataIngestionService;
    let mockRawRepository;
    let mockDataSourceRepository;
    let mockProcessedDataService;
    ...
    beforeEach(() => {
        ...
        rawDataIngestionService = new RawDataIngestionService(mockRawRepository, mockDataSourceRepository, mockProcessedDataService);
        rawDataIngestionService.providers = {
    ...
    describe("ingestData()", () => {
        it("harus berhasil menginjeksi data OHLCV, membuat raw record, dan memproses data", async () => {
            ...
            // Act
            const result = await rawDataIngestionService.ingestData("binance", "ds-uuid-123", "BTCUSDT", { interval: "1d" });
            // Assert
            expect(mockDataSourceRepository.findById).toHaveBeenCalledWith("ds-uuid-123");
            expect(mockBinanceProvider.getOHLCV).toHaveBeenCalledWith("BTCUSDT", { interval: "1d" }, {
                baseURL: "https://api.binance.com/api/v3",
                connection: "",
            });
            ...
            expect(mockProcessedDataService.processRawData).toHaveBeenCalledWith("raw-uuid-123");
        ...
        it("harus melempar error jika DataSource tidak ditemukan", async () => {
            ...
            // Act & Assert
            await expect(rawDataIngestionService.ingestData("binance", "non-existent-ds", "BTCUSDT")).rejects.toThrow("DataSource dengan ID non-existent-ds tidak ditemukan.");
        });
        it("harus melempar error jika DataSource tidak aktif", async () => {
            ...
            // Act & Assert
            await expect(rawDataIngestionService.ingestData("binance", "ds-uuid-123", "BTCUSDT")).rejects.toThrow("DataSource 'Binance Spot Public' (ID: ds-uuid-123) sedang tidak aktif / dinonaktifkan.");
        });
        it("harus melempar error jika provider tidak didukung", async () => {
            // Arrange
            mockDataSourceRepository.findById.mockResolvedValue(mockDataSourceRecord);
            // Act & Assert
            await expect(rawDataIngestionService.ingestData("unsupported", "ds-uuid-123", "BTCUSDT")).rejects.toThrow("Provider 'unsupported' tidak didukung.");
        });
        it("harus mencetak console.error dan melempar error jika provider getOHLCV melempar error", async () => {
            ...
            const consoleErrorSpy = vi
                ...
                .mockImplementation(() => { });
            // Act & Assert
            await expect(rawDataIngestionService.ingestData("binance", "ds-uuid-123", "BTCUSDT")).rejects.toThrow("API Network error");
    ...
    describe("getRawDataByDatasource()", () => {
        it("harus mengembalikan RawData berdasarkan datasource ID", async () => {
            ...
            // Act
            const result = await rawDataIngestionService.getRawDataByDatasource("ds-uuid-123", 5);
            // Assert
            expect(mockRawRepository.findByDatasource).toHaveBeenCalledWith("ds-uuid-123", 5);
    ...
    describe("updateRawStatus()", () => {
        it("harus memperbarui status RawData jika record ditemukan", async () => {
            ...
            // Act
            const result = await rawDataIngestionService.updateRawStatus("raw-uuid-123", RawDataStatus.SUCCESS);
            // Assert
            expect(mockRawRepository.exists).toHaveBeenCalledWith("raw-uuid-123");
            expect(mockRawRepository.updateStatus).toHaveBeenCalledWith("raw-uuid-123", RawDataStatus.SUCCESS);
        ...
        it("harus melempar error jika RawData tidak ditemukan saat update status", async () => {
            ...
            // Act & Assert
            await expect(rawDataIngestionService.updateRawStatus("non-existent-id", RawDataStatus.SUCCESS)).rejects.toThrow("RawData dengan ID non-existent-id tidak ditemukan");
        });
    });
});
//# sourceMappingURL=raw-data.test.js.map