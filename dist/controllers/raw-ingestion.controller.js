...
export class RawIngestionCtr {
    ingestionService;
    symbolRepository;
    constructor(ingestionService = new RawDataIngestionService(), symbolRepository = new SymbolRepository()) {
    ...
    ingestData = async (req, res) => {
        try {
            ...
            const symbolRecord = await this.symbolRepository.findByDataSourceAndTicker(dataSourceId, symbol);
            ...
            if (!symbolRecord || !symbolRecord.isActive) {
                throw new Error(`Symbol '${symbol}' tidak ditemukan atau sedang non-aktif pada DataSource ini.`);
            }
            const result = await this.ingestionService.ingestData(provider, dataSourceId, symbol, { interval, range });
            res.status(201).json({
                success: true,
...
//# sourceMappingURL=raw-ingestion.controller.js.map