...
describe("DataSourceService", () => {
  ...
  describe("getDataSourcesById()", () => {
    it("harus mengembalikan DataSource berdasarkan ID", async () => {
      ...
      // Assert
      expect(mockDataSourceRepository.findById).toHaveBeenCalledWith(
        "ds-uuid-123",
      );
      expect(result).toEqual(mockDataSourceRecord);
    });
  ...
  describe("getDataSourcesByName()", () => {
    it("harus mengembalikan DataSource berdasarkan Name", async () => {
      // Arrange
      mockDataSourceRepository.findByName.mockResolvedValue(
        mockDataSourceRecord,
      );

      // Act
      const result = await dataSourceService.getDataSourcesByName(
        "Binance Spot Public",
      );

      // Assert
      expect(mockDataSourceRepository.findByName).toHaveBeenCalledWith(
        "Binance Spot Public",
      );
      expect(result).toEqual(mockDataSourceRecord);
    });
  ...
  describe("updateDataSources()", () => {
    it("harus memperbarui status isActive DataSource", async () => {
      ...
      // Act
      const result = await dataSourceService.updateDataSources(
        "ds-uuid-123",
        false,
      );

      // Assert
      expect(mockDataSourceRepository.update).toHaveBeenCalledWith(
        "ds-uuid-123",
        false,
      );
